...
class Site(DnacBase):
    ...
    def is_floor_updated(self, updated_site, requested_site):
        ...
        Returns:
            bool: True if both floor details match, False if there is any difference.
        """
        if self.compare_dnac_versions(self.get_ccc_version(), "2.3.5.3") <= 0:
            return (
                        ...
                        if task_id:
                            ...
                            for site in self.want:
                                if site.get("type") == "floor":
                                    self.log("Floor '{}' found. Proceeding to upload floor map.".format(site.get("name")), "INFO")
        ...
        if self.have.get("site_exists"):
            ...
            if self.compare_dnac_versions(self.get_ccc_version(), "2.3.5.3") <= 0:
                try:
                    ...
                    self.log("Unexpected error occurred: {0}".format(str(e)), "ERROR")
                    error_message = "Failed to create site '{0}': {1}".format(site_name_hierarchy, str(e))
                    return {"error_message": error_message}
                ...
                if (site_created or site_updated) and response and isinstance(response, dict):
                    ...
                    if site_exists:
                        site_name_hierarchy = self.want.get("site_name_hierarchy")
                        self.created_site_list.append(site_name_hierarchy)
                        self.log("Site '{0}' created successfully".format(site_name_hierarchy), "INFO")
                    return self
            else:
    ...
    def delete_floor(self, site_name_hierarchy):
        """
    ...
    def delete_building(self, site_name_hierarchy):
        """
    ...
    def delete_area(self, site_name_hierarchy):
        """
    ...
    def get_diff_deleted(self, config):
        ...
        if self.compare_dnac_versions(self.get_ccc_version(), "2.3.5.3") <= 0:
            ...
            response = None
            if site_type == "floor":
                response = self.delete_floor(site_name_hierarchy)
            elif site_type == "area":
                response = self.delete_area(site_name_hierarchy)
                self.log("Response for deleting area: {0}".format(str(response)), "DEBUG")
            elif site_type == "building":
                response = self.delete_building(site_name_hierarchy)
    ...
    def upload_floor_image(self, config):
        ...
        try:
            ...
            if not isinstance(file_path, str) or not file_path:
                raise ValueError("Invalid file path format. It must be a non-empty string.")
            ...
            if not any(file_path.lower().endswith(ext) for ext in valid_extensions):
                raise ValueError("Unsupported file format. Supported formats: {}".format(", ".join(valid_extensions)))
